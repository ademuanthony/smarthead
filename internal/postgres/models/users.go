...
// TeacherTimetables retrieves all the timetable's Timetables with an executor via teacher_id column.
func (o *User) TeacherTimetables(mods ...qm.QueryMod) timetableQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"timetable\".\"teacher_id\"=?", o.ID),
	)
...
// LoadTeacherTimetables allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadTeacherTimetables(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`timetable`), qm.WhereIn(`timetable.teacher_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.TeacherTimetables = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.TeacherID {
				local.R.TeacherTimetables = append(local.R.TeacherTimetables, foreign)
				if foreign.R == nil {
					foreign.R = &timetableR{}
				}
...
// AddTeacherTimetables adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.TeacherTimetables.
// Sets related.R.Teacher appropriately.
func (o *User) AddTeacherTimetables(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Timetable) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.TeacherID = o.ID
	...
	if o.R == nil {
		o.R = &userR{
			TeacherTimetables: related,
		}
	} else {
		o.R.TeacherTimetables = append(o.R.TeacherTimetables, related...)
	}
