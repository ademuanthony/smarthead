...
func (o *Subject) Timetables(mods ...qm.QueryMod) timetableQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.Where("\"timetable\".\"subject_id\"=?", o.ID),
	)
...
func (subjectL) LoadStudents(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSubject interface{}, mods queries.Applicator) error {
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Name, &one.Username, &one.RegNo, &one.Age, &one.AccountBalance, &one.CurrentClass, &one.ParentPhone, &one.ParentEmail, &one.CreatedAt, &one.UpdatedAt, &one.ClassID, &one.SubclassID, &one.LastPaymentDate, &localJoinCol)
		if err != nil {
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (subjectL) LoadTimetables(ctx context.Context, e boil.ContextExecutor, singular bool, maybeSubject interface{}, mods queries.Applicator) error {
	var slice []*Subject
	var object *Subject
	...
	query := NewQuery(qm.From(`timetable`), qm.WhereIn(`timetable.subject_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
...
// Sets related.R.Subject appropriately.
func (o *Subject) AddTimetables(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Timetable) error {
	var err error
	for _, rel := range related {
		if insert {
			rel.SubjectID = o.ID
