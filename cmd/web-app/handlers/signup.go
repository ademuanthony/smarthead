...
func (h *Signup) GetStarted(ctx context.Context, w http.ResponseWriter, r *http.Request, params map[string]string) error {
	...
	if len(timetables) < 2 {
		...
		err = h.EmailNotifier.Send(ctx, req.Email, "Welcome to Smart Head", "welcome_email", data)
		if err != nil {
			return err
		}
		err = h.EmailNotifier.Send(ctx, "onlineremoteschool@gmail.com", "Welcome to Smart Head for "+req.Name, "welcome_email", data)
		if err != nil {
			return err
		}
